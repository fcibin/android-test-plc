...
public class PLCinterface
{
	...
	/*
	 * public int read(byte[] b, int start, int len) { int res; if
	 * ((Nodave.Debug & Nodave.DEBUG_IFACE) != 0)
	 * System.out.println("Interface.read"); try { res = in.read(b, start, len);
	 * System.out.println(res+" bytes read"); return res; } catch (IOException
	 ...
	 * public int read(byte[] b, int start, int len) { int res; if
	 * ((Nodave.Debug & Nodave.DEBUG_IFACE) != 0)
	 * System.out.println("Interface.read"); try { int retry = 0; while
	 * ((in.available() <= 0) && (retry < 20)) { try { Thread.sleep(timeout /
	 * 20); retry++; if ((Nodave.Debug & Nodave.DEBUG_IFACE) != 0)
	 * System.out.println("Interface.read delayed"); } catch
	 * (InterruptedException e) { System.out.println(e); } }
	 * 
	 * if (in.available() > 0) { // if ((Nodave.Debug & Nodave.DEBUG_IFACE) !=
	 * 0) // System.out.println("can read"); res = in.read(b, start, len); //
	 * System.out.println(res+" bytes read"); return res; } return 0; } catch
	 * (IOException e) { System.out.println(e); return 0; } }
	 */
							...
							try
								{
									...
									if ((Nodave.Debug & Nodave.DEBUG_IFACE) != 0)
										System.out
												.println("Interface.read delayed");
