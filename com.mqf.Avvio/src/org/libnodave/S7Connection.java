	...
	public int readBytes(int area, int DBnum, int start, int len, byte[] buffer)
		{
			...
			if ((Nodave.Debug & Nodave.DEBUG_CONN) != 0)
				System.out.println("setupReceivedPDU() returned: " + res
						+ Nodave.strerror(res));
			if (res != Nodave.RESULT_OK)
				{
					semaphore.leave();
			...
			if ((Nodave.Debug & Nodave.DEBUG_CONN) != 0)
				System.out.println("testReadResult() returned: " + res
						+ Nodave.strerror(res));
			if (res != Nodave.RESULT_OK)
				{
					semaphore.leave();
							...
							if (r.error == 255)
								{
									...
									// System.arraycopy(p2.mem, pos + 4, r.data,
									// 0, len);
									// Nodave.dump("Result " + i + ":", r.data,
									// 0, len);
									r.bufferStart = pos + 4;
									pos += len;
									if ((len % 2) == 1)
	...
	public int negPDUlengthRequest()
		{
			...
			if ((Nodave.Debug & Nodave.DEBUG_CONNECT) != 0)
				{
					System.out.println("*** Partner offered PDU length: "
							+ maxPDUlength);
				}
			return res;
		}
