...
public class PDU
{
	...
	public int addVarToReadRequest(int area, int DBnum, int start, int len)
		{
			...
			if ((area == Nodave.ANALOGINPUTS200)
					|| (area == Nodave.ANALOGOUTPUTS200))
				{
					pa[3] = 4;
					start *= 8; /* bits */
				} else if ((area == Nodave.TIMER) || (area == Nodave.COUNTER)
					|| (area == Nodave.TIMER200) || (area == Nodave.COUNTER200))
				{
					pa[3] = (byte) area;
				} else
				{
			...
			/**
			 * TODO calc length of result. Do not add variable if it would
			 * exceed max. result length.
			 */
			return 0;
		}
	...
	void addValue(byte[] values)
		{
			...
			if (mem[data + 1] == 4)
				...
				{
					if ((Nodave.Debug & Nodave.DEBUG_PDU) != 0)
						System.out.println("unknown data type/length: "
								+ mem[data + 1]);
				}
			if (udata == 0)
				udata = data + 4;
	...
	public int setupReceivedPDU()
		{
			int res = Nodave.RESULT_CANNOT_EVALUATE_PDU; // just assume the
															// worst
	...
	*/
			...
			if ((mem[data] == (byte) 255) && (dlen > 4))
				{
					...
					if (mem[data + 1] == 4)
						...
						{
							/*
							 * len is in bits, but there is a byte per result
							 * bit, ok
							 */
						} else
						{
							if ((Nodave.Debug & Nodave.DEBUG_PDU) != 0)
								System.out
										.println("fixme: what to do with data type "
												+ mem[data + 1]);
	...
	public void addVarToWriteRequest(int area, int DBnum, int start,
			int byteCount, byte[] buffer)
		{
			...
			if ((area == Nodave.TIMER) || (area == Nodave.COUNTER)
					|| (area == Nodave.TIMER200) || (area == Nodave.COUNTER200))
				{
					pa[3] = (byte) area;
	...
	public void addBitVarToWriteRequest(int area, int DBnum, int start,
			int byteCount, byte[] buffer)
		{
			byte da[] = { 0, 3, 0, 0, };
